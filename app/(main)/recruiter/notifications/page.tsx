...
export default function RecruiterNotificationsPage() {
    ...
    const markReadMutation = useMutation({
        ...
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ["recruiter-notifications"] }),
    });
            ...
            {isLoading ? (
                ...
                <>
                    ...
                    <div className="space-y-2">
                        {notifications?.map((n) => (
                            <Card key={n.ID} onClick={() => handleClick(n)} className={`cursor-pointer border p-0 shadow-sm transition-all hover:shadow-md ${n.IsRead ? "border-gray-100 bg-white" : "border-blue-100 bg-blue-50/50"}`}>
                                <div className="flex items-center gap-4 px-5 py-4">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <p className={`text-sm line-clamp-1 ${n.IsRead ? "text-gray-600" : "font-semibold text-gray-900"}`}>{n.Notification?.Title || "Thông báo"}</p>
                                        <div className="mt-1 flex items-center gap-1 text-xs text-gray-400"><Clock size={12} />{formatDate(n.CreatedAt)}</div>
                                    </div>
                                    <Button variant="ghost" size="icon" className="flex-shrink-0 text-gray-300 hover:bg-red-50 hover:text-red-500" onClick={(e) => { e.stopPropagation(); deleteMutation.mutate(n.ID); }}>
                                        <Trash2 size={16} />
                                    </Button>
                                </div>
                            </Card>
            ...
            <Sheet open={!!selectedNotification} onOpenChange={() => setSelectedNotification(null)}>
                <SheetContent className="sm:max-w-md">
                    <SheetHeader>
                        ...
                        <SheetDescription>{selectedNotification?.CreatedAt && formatDate(selectedNotification.CreatedAt)}</SheetDescription>
                    </SheetHeader>
                    ...
                    <div className="mt-8">
                        <Button onClick={() => setSelectedNotification(null)} className="w-full bg-[#194d8e] hover:bg-[#194d8e]/90">Đóng</Button>
                    </div>
                </SheetContent>
            </Sheet>
        </div>
